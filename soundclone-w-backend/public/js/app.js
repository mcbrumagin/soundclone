...
document.addEventListener('DOMContentLoaded', function() {
        ...
        try {
            ...
            trackListElement.innerHTML = `
                <div class="error-message">
                    ...
                    <button id="retryLoadTracks">Retry</button>
                </div>
            `;
            
            document.getElementById('retryLoadTracks').addEventListener('click', loadTracks);
        }
    }
    ...
    function renderTrackList(tracks) {
        ...
        if (tracks.length === 0) {
            trackListElement.innerHTML = '<div class="empty-state">No tracks found. Upload or record something!</div>';
            return;
        }
    ...
    async function loadTrackDetails(trackId) {
        try {
            trackDetailElement.innerHTML = '<div class="loading-indicator">Loading track details...</div>';
            ...
            trackDetailElement.innerHTML = `
                <div class="error-message">
                    <p>Failed to load track details. Please try again later.</p>
                    <button id="retryLoadTrackDetails">Retry</button>
                </div>
            `;
            
            document.getElementById('retryLoadTrackDetails').addEventListener('click', () => loadTrackDetails(trackId));
        }
    }
    ...
    function renderComments(track) {
        ...
        if (track.comments.length === 0) {
            commentListElement.innerHTML = '<div class="empty-state">No comments yet. Be the first to comment!</div>';
            return;
        }
    ...
    function setupTrackDetailEventListeners(track) {
        ...
        if (editButton) {
            editButton.addEventListener('click', function() {
                ...
                if (newTitle !== null && newTitle.trim() !== '') {
                    ...
                    ApiService.updateTrack(track.id, {
                        title: newTitle,
                        description: newDescription !== null ? newDescription : track.description
                    })
                    .then(updatedTrack => {
                        // Refresh track details
                        renderTrackDetail(updatedTrack);
                    })
                    .catch(error => {
        ...
        if (deleteButton) {
            deleteButton.addEventListener('click', function() {
                if (confirm('Are you sure you want to delete this track?')) {
                    // Delete track via API
                    ApiService.deleteTrack(track.id)
                    .then(() => {
                        // Navigate back to home and refresh track list
                        navigateToView('home');
                        loadTracks();
                    })
                    .catch(error => {
        ...
        addCommentButton.addEventListener('click', function() {
            ...
            if (commentText) {
                // Add comment via API
                ApiService.addComment(track.id, commentText)
                .then(newComment => {
                    // Clear input
                    commentInput.value = '';
                    
                    // Refresh track details to show new comment
        ...
        commentListElement.addEventListener('click', function(e) {
            ...
            if (editButton) {
                ...
                if (newText !== null && newText.trim() !== '') {
                    ...
                    ApiService.updateComment(track.id, commentId, newText)
                    .then(() => {
                        // Refresh track details to show updated comment
                        loadTrackDetails(track.id);
                    })
                    .catch(error => {
            ...
            if (deleteButton) {
                ...
                if (confirm('Are you sure you want to delete this comment?')) {
                    ...
                    ApiService.deleteComment(track.id, commentId)
                    .then(() => {
                        // Refresh track details to remove deleted comment
                        loadTrackDetails(track.id);
                    })
                    .catch(error => {
    ...
    function setupUploadViewEventListeners() {
        ...
        uploadButton.addEventListener('click', function() {
            ...
            formData.append('audio', appState.selectedFile);
            ...
            ApiService.uploadTrack(formData)
            .then(track => {
                // Reset form
                fileInput.value = '';
                ...
                // Navigate to home and refresh track list
